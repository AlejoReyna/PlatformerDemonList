...
class SubmitRecordView(TemplateView):
    ...
    def post(self, request):
        ...
        enjoyment_stars = r.get("enjoyment_stars", None)
        gameplay_stars = r.get("gameplay_stars", None)
        decoration_stars = r.get("decoration_stars", None)
        balanced_stars = r.get("balanced_stars", None)
        atmosphere_stars = r.get("atmosphere_stars", None)

        time = datetime.time(int(hours), int(minutes), int(seconds), int(milliseconds) * 1000)
        
        print(r)

        demon = Demon.objects.get(level=demon)

        Record.objects.create(demon=demon,
                            best_time=time,
                            player=Profile.objects.get(id=profile),
                            video=video,
                            raw_footage=raw_footage,
                            notes=notes,
                            enjoyment_stars=enjoyment_stars,
                            gameplay_stars=gameplay_stars,
                            decoration_stars=decoration_stars,
                            balanced_stars=balanced_stars,
                            atmosphere_stars=atmosphere_stars,
                            )
    ...
    def post(self, request):
        ...
        if r.get("accept_id", None):
            ...
            try:
                ...
                functions.update_top_order(old_record.demon)
                functions.update_top_best_time(old_record.demon)
                functions.update_players_list_points(old_record.player)
                functions.update_countries_list_points(old_record.player.country)
                functions.upload_all_stars(old_record.demon)
            except:
...
class AddEditDemonView(LoginRequiredMixin, ModeradorMixin, TemplateView):
    ...
    def post(self, request):
        ...
        if r.get("option", None) == "Add":
            ...
            level_id_ldm = r.get("level_id_ldm", None)
            ...
            if r.get("level_password", None):
                new_demon.level_id_ldm = level_id_ldm
        ...
        if r.get("option", None) == "Edit":
            ...
            old_demon.level_id_ldm=level_id_ldm
...
class RouletteView(DetailView):
    """Roulette view."""

    template_name = 'demonlist/roulette.html'
    queryset = Roulette.objects.all()
    slug_field = 'id'
    slug_url_kwarg = 'roulette'
    context_object_name = "roulette"
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user'] = self.request.user
        context['demon'] = Demon.objects.get(level="I wanna be the guy")
        context['demons'] = demons
        return context
    ...
    def post(self, request):
        ...
        number_demons = r.get("number_demons", None)
        another = r.get("another", None)
        print(r)

        if not(another):

            demon = Demon.objects.filter(demon_difficulty="Easy demon")

            random_number = random.randint(1, len(demon))

            demon = demon[random_number - 1]

            roulette = Roulette.objects.create(name=name,
                                            player=self.request.user.profile,
                                            max_demons=int(number_demons)*5)
            
            roulette.demons.add(demon)

            demon = {"level": demon.level,
                    "verification_video": demon.verification_video,
                    "verification_video": demon.verification_video,
                    "photo__url": demon.photo.url,
                    "position": demon.position,
                    "creator": demon.creator,
                    }

            return JsonResponse(demon, safe=False)
        else:

            roulette = Roulette.objects.get(id=id)
...
class SelectRouletteView(TemplateView):
    """SelectRoulette view."""

    template_name = 'demonlist/select_roulette.html'
    ...
    def post(self, request):
        r = request.POST
        roulette = r.get("roulette", None)
        
        print(r)

        if roulette == "create":
            return HttpResponseRedirect(reverse_lazy('demonlist:roulette'))